      ...
      <aside class="main-sidebar sidebar-dark-primary elevation-4">
        ...
        <div class="sidebar">
          ...
          <div class="user-panel mt-3 pb-3 mb-3 d-flex">
            <div class="image">
              <img
                src="../dist/img/kosong.jpeg"
                class="img-circle elevation-2"
                alt="User Image"
              />
            </div>
            <div class="info">
              <a href="#" class="d-block" style="font-size: 20px"><?php echo $infoUser['nama'] ?></a>
            </div>
          </div>
...
<table >
        ...
        ?>
            <tr>
                ...
                <?php
                $query_h = mysqli_query($koneksi, "SELECT COUNT(id_status) as h from tb_absensi where year(tanggal) ='$tahun' and  MONTH(tanggal) = '$bulan' and id_status = 'H' and id_siswa = '" . $rowSiswa['nis'] . "' ");
                $H = mysqli_fetch_assoc($query_h);
                $query_i = mysqli_query($koneksi, "SELECT COUNT(id_status) as i from tb_absensi where year(tanggal) ='$tahun' and MONTH(tanggal) = '$bulan' and id_status = 'I' and id_siswa = '" . $rowSiswa['nis'] . "' ");
                $I = mysqli_fetch_assoc($query_i);
                $query_s = mysqli_query($koneksi, "SELECT COUNT(id_status) as s from tb_absensi where year(tanggal) ='$tahun' and MONTH(tanggal) = '$bulan' and id_status = 's' and id_siswa = '" . $rowSiswa['nis'] . "' ");
                $S = mysqli_fetch_assoc($query_s);
                $query_a = mysqli_query($koneksi, "SELECT COUNT(id_status) as a from tb_absensi where year(tanggal) ='$tahun' and MONTH(tanggal) = '$bulan' and id_status = 'a' and id_siswa = '" . $rowSiswa['nis'] . "' ");
